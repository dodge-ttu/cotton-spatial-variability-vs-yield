...
directory_path = os.path.join(output_dir, "{0}-{1}-shapefiles".format(planting, what))
if not os.path.exists(directory_path):
    os.makedirs(directory_path)

# Change layer CRS for a list of layers:
def change_projections(layer_list, **kwargs):

    for layer in a_layer_of_points:

        parameters = {
            'layer': layer,
            'fileName': os.path.join(directory_path, "{0}_process".format(layer.name())),
            'fileEncoding': "utf-8",
...
params = {'output_path':directory_path}

change_projections(a_layer_of_points)
